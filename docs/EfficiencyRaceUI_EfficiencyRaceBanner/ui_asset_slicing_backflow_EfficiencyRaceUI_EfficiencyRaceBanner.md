# EfficiencyRaceUI_EfficiencyRaceBanner UI 切图回流规范

> 对应 prefab：`Assets/Prefabs/UI/Panels/EfficiencyRaceUI_EfficiencyRaceBanner.prefab`  
> 参考原型：`ui_layout_prototype_EfficiencyRaceUI_EfficiencyRaceBanner.png` / `ui_layout_prototype_EfficiencyRaceUI_EfficiencyRaceBanner.svg`  
> 坐标基准：1080 x 1920，文档坐标为左上角 x/y，Unity 回流时按 RectTransform 中心点换算。  
> 建议导入目录：`Assets/Art/UI/Generated/EfficiencyRaceUI_EfficiencyRaceBanner/`
...
| --- | --- | --- | --- | --- | --- | --- |
| 游戏安全区 | `SceneSafeArea` | 40, 250, 1000, 1260 | `-` | 不单独切图；作为游戏画面/安全区参考 | Center，pivot=(0.5,0.5)，sizeDelta=(1000,1260)，anchoredPosition=(0,80) | 无文案切图 |
| 竞速条 | `BannerRoot` | 110, 135, 860, 105 | `ui_EfficiencyRaceUI_EfficiencyRaceBanner__02_BannerRoot.png` | 切透明 PNG，建议九宫格 Sprite，Image Type=Sliced | TopCenter，pivot=(0.5,0.5)，sizeDelta=(860,105)，anchoredPosition=(0,773) | 静态装饰可烘焙；动态数字/昵称/状态不要烘焙 |
| 对比文案 | `MessageText` | 150, 165, 780, 46 | `ui_EfficiencyRaceUI_EfficiencyRaceBanner__03_MessageText_bg.png` | 保留 TMP/按钮文字动态层；只切底板、图标或装饰 | TopCenter，pivot=(0.5,0.5)，sizeDelta=(780,46)，anchoredPosition=(0,772) | 文字不烘焙进切图，继续由 TMP/脚本字段驱动 |

## 4. 回流步骤

1. 按 `ai_prompt.md` 和原型 PNG 生成效果图后，先拆出不含动态文字的透明 PNG 切片。
2. 将切片放入 `Assets/Art/UI/Generated/EfficiencyRaceUI_EfficiencyRaceBanner/`，按本文件的建议文件名命名。
3. 在 Unity Import Settings 中套用上面的 Sprite 设置；需要拉伸的底板进入 Sprite Editor 设置 Border。
4. 打开 `Assets/Prefabs/UI/Panels/EfficiencyRaceUI_EfficiencyRaceBanner.prefab`，按“Prefab/节点”列定位节点，优先替换现有 Image.sprite / RawImage.texture。
...
| 竞速条 | `BannerRoot` | 现有节点命中 | `BannerRoot` | 在对应现有节点上替换 Image.sprite / RawImage.texture；保留其子级 TMP、Button、脚本引用和显隐逻辑。 |
| 对比文案 | `MessageText` | 现有节点命中 | `MessageText` | 在对应现有节点上替换 Image.sprite / RawImage.texture；保留其子级 TMP、Button、脚本引用和显隐逻辑。 |
