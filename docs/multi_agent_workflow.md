...
| BroadcasterPanel 字段未绑定 | Setup 脚本内"**缺 GO 就建占位按钮 + 自动绑定 SerializedField**"（不跳过、不让用户手动） | 完整覆盖 §36.12 全部 feature id 的视觉门禁 |
...
| 服务端改动部署 | `Bash python deploy.py`（主 repo 下）+ PM2 自动重启 | **PM 自行部署**，不让用户跑 |
...
- ❌ PM 要求用户"自己跑 deploy.py"（应该 PM 自己 `Bash python deploy.py`）
- ❌ Setup 脚本遇到 Inspector 字段未绑定就跳过，留给用户手动（应该**自动建占位 GO + 绑回 SerializedField**，保证功能完整）
