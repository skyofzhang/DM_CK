# FeatureUnlockBanner UI 切图回流规范

> 对应 prefab：`Assets/Prefabs/UI/Panels/FeatureUnlockBanner.prefab`  
> 参考原型：`ui_layout_prototype_FeatureUnlockBanner.png` / `ui_layout_prototype_FeatureUnlockBanner.svg`  
> 坐标基准：1080 x 1920，文档坐标为左上角 x/y，Unity 回流时按 RectTransform 中心点换算。  
> 建议导入目录：`Assets/Art/UI/Generated/FeatureUnlockBanner/`
...
| --- | --- | --- | --- | --- | --- | --- |
| 游戏安全区 | `SceneSafeArea` | 40, 300, 1000, 1210 | `-` | 不单独切图；作为游戏画面/安全区参考 | Center，pivot=(0.5,0.5)，sizeDelta=(1000,1210)，anchoredPosition=(0,55) | 无文案切图 |
| 解锁横幅 | `FeatureUnlockBanner` | 95, 170, 890, 180 | `ui_FeatureUnlockBanner__02_FeatureUnlockBanner.png` | 切透明 PNG，建议九宫格 Sprite，Image Type=Sliced | TopCenter，pivot=(0.5,0.5)，sizeDelta=(890,180)，anchoredPosition=(0,700) | 静态装饰可烘焙；动态数字/昵称/状态不要烘焙 |
| 标题 | `TitleText` | 145, 205, 320, 54 | `ui_FeatureUnlockBanner__03_TitleText_bg.png` | 保留 TMP/按钮文字动态层；只切底板、图标或装饰 | TopLeft，pivot=(0.5,0.5)，sizeDelta=(320,54)，anchoredPosition=(-235,728) | 文字不烘焙进切图，继续由 TMP/脚本字段驱动 |
| 说明 | `DescText` | 145, 280, 760, 54 | `ui_FeatureUnlockBanner__04_DescText_bg.png` | 保留 TMP/按钮文字动态层；只切底板、图标或装饰 | TopCenter，pivot=(0.5,0.5)，sizeDelta=(760,54)，anchoredPosition=(-15,653) | 文字不烘焙进切图，继续由 TMP/脚本字段驱动 |

## 4. 回流步骤

1. 按 `ai_prompt.md` 和原型 PNG 生成效果图后，先拆出不含动态文字的透明 PNG 切片。
2. 将切片放入 `Assets/Art/UI/Generated/FeatureUnlockBanner/`，按本文件的建议文件名命名。
3. 在 Unity Import Settings 中套用上面的 Sprite 设置；需要拉伸的底板进入 Sprite Editor 设置 Border。
4. 打开 `Assets/Prefabs/UI/Panels/FeatureUnlockBanner.prefab`，按“Prefab/节点”列定位节点，优先替换现有 Image.sprite / RawImage.texture。
...
| 解锁横幅 | `FeatureUnlockBanner` | 现有节点命中 | `FeatureUnlockBanner` | 在对应现有节点上替换 Image.sprite / RawImage.texture；保留其子级 TMP、Button、脚本引用和显隐逻辑。 |
