# StatusLineBannerUI_StatusLineBanner UI 切图回流规范

> 对应 prefab：`Assets/Prefabs/UI/Panels/StatusLineBannerUI_StatusLineBanner.prefab`  
> 参考原型：`ui_layout_prototype_StatusLineBannerUI_StatusLineBanner.png` / `ui_layout_prototype_StatusLineBannerUI_StatusLineBanner.svg`  
> 坐标基准：1080 x 1920，文档坐标为左上角 x/y，Unity 回流时按 RectTransform 中心点换算。  
> 建议导入目录：`Assets/Art/UI/Generated/StatusLineBannerUI_StatusLineBanner/`
...
| --- | --- | --- | --- | --- | --- | --- |
| 游戏安全区 | `SceneSafeArea` | 40, 210, 1000, 1260 | `-` | 不单独切图；作为游戏画面/安全区参考 | Center，pivot=(0.5,0.5)，sizeDelta=(1000,1260)，anchoredPosition=(0,120) | 无文案切图 |
| 状态细条 | `BannerRoot` | 70, 105, 940, 72 | `ui_StatusLineBannerUI_StatusLineBanner__02_BannerRoot_bg.png` | 保留 TMP/按钮文字动态层；只切底板、图标或装饰 | TopCenter，pivot=(0.5,0.5)，sizeDelta=(940,72)，anchoredPosition=(0,819) | 文字不烘焙进切图，继续由 TMP/脚本字段驱动 |
| 状态文案 | `StatusText` | 105, 125, 870, 36 | `ui_StatusLineBannerUI_StatusLineBanner__03_StatusText_bg.png` | 保留 TMP/按钮文字动态层；只切底板、图标或装饰 | TopCenter，pivot=(0.5,0.5)，sizeDelta=(870,36)，anchoredPosition=(0,817) | 文字不烘焙进切图，继续由 TMP/脚本字段驱动 |

## 4. 回流步骤

1. 按 `ai_prompt.md` 和原型 PNG 生成效果图后，先拆出不含动态文字的透明 PNG 切片。
2. 将切片放入 `Assets/Art/UI/Generated/StatusLineBannerUI_StatusLineBanner/`，按本文件的建议文件名命名。
3. 在 Unity Import Settings 中套用上面的 Sprite 设置；需要拉伸的底板进入 Sprite Editor 设置 Border。
4. 打开 `Assets/Prefabs/UI/Panels/StatusLineBannerUI_StatusLineBanner.prefab`，按“Prefab/节点”列定位节点，优先替换现有 Image.sprite / RawImage.texture。
...
| 状态细条 | `BannerRoot` | 施工目标需创建/改名 | - | 当前 prefab 未发现同名 m_Name。回流时可在最近的稳定父节点下新增或改名为该施工目标的 Image 容器，再按本表坐标设置 RectTransform；不要替换 TMP、Button 或脚本绑定对象。 |
