# GiftImpactUI_GiftImpactBanner UI 切图回流规范

> 对应 prefab：`Assets/Prefabs/UI/Panels/GiftImpactUI_GiftImpactBanner.prefab`  
> 参考原型：`ui_layout_prototype_GiftImpactUI_GiftImpactBanner.png` / `ui_layout_prototype_GiftImpactUI_GiftImpactBanner.svg`  
> 坐标基准：1080 x 1920，文档坐标为左上角 x/y，Unity 回流时按 RectTransform 中心点换算。  
> 建议导入目录：`Assets/Art/UI/Generated/GiftImpactUI_GiftImpactBanner/`
...
| --- | --- | --- | --- | --- | --- | --- |
| 游戏安全区 | `SceneSafeArea` | 40, 230, 1000, 1150 | `-` | 不单独切图；作为游戏画面/安全区参考 | Center，pivot=(0.5,0.5)，sizeDelta=(1000,1150)，anchoredPosition=(0,155) | 无文案切图 |
| 礼物影响横幅 | `GiftImpactBanner` | 170, 1330, 740, 140 | `ui_GiftImpactUI_GiftImpactBanner__02_GiftImpactBanner.png` | 切透明 PNG，建议九宫格 Sprite，Image Type=Sliced | BottomCenter，pivot=(0.5,0.5)，sizeDelta=(740,140)，anchoredPosition=(0,-440) | 静态装饰可烘焙；动态数字/昵称/状态不要烘焙 |
| 影响文案 | `ImpactText` | 210, 1370, 660, 56 | `ui_GiftImpactUI_GiftImpactBanner__03_ImpactText_bg.png` | 保留 TMP/按钮文字动态层；只切底板、图标或装饰 | BottomCenter，pivot=(0.5,0.5)，sizeDelta=(660,56)，anchoredPosition=(0,-438) | 文字不烘焙进切图，继续由 TMP/脚本字段驱动 |

## 4. 回流步骤

1. 按 `ai_prompt.md` 和原型 PNG 生成效果图后，先拆出不含动态文字的透明 PNG 切片。
2. 将切片放入 `Assets/Art/UI/Generated/GiftImpactUI_GiftImpactBanner/`，按本文件的建议文件名命名。
3. 在 Unity Import Settings 中套用上面的 Sprite 设置；需要拉伸的底板进入 Sprite Editor 设置 Border。
4. 打开 `Assets/Prefabs/UI/Panels/GiftImpactUI_GiftImpactBanner.prefab`，按“Prefab/节点”列定位节点，优先替换现有 Image.sprite / RawImage.texture。
...
| 礼物影响横幅 | `GiftImpactBanner` | 现有节点命中 | `GiftImpactUI_GiftImpactBanner` | 在对应现有节点上替换 Image.sprite / RawImage.texture；保留其子级 TMP、Button、脚本引用和显隐逻辑。 |
| 影响文案 | `ImpactText` | 现有节点命中 | `ImpactText` | 在对应现有节点上替换 Image.sprite / RawImage.texture；保留其子级 TMP、Button、脚本引用和显隐逻辑。 |
