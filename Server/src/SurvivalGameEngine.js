...
class SurvivalGameEngine {
  ...
  _enterNight(day) {
    ...
    if (cfg.boss && !this._peaceNightSkipSpawn && !suppressRegularBoss) {
      ...
      //   codex review §34.4 E2：保留 _actEndHpBossMult 幕末乘数；boss 注册到 _activeMonsters Map 走 boss_appeared 路径
      const _earlyMultBoss = this._getEarlyDayMult(this.fortressDay);
      const _bossSpawnHp = Math.max(1, Math.round(cfg.boss.hp * (this._dynamicHpMult || 1.0) * (this._themeHpMult || 1.0) * (this._themeBossHpMult || 1.0) * (this._actEndHpBossMult || 1.0) * _earlyMultBoss));
  ...
  _initActiveMonsters(day) {
    ...
    // §14 v1.27 + codex review：_initActiveMonsters 仅做 ledger 预算日志（不再写 Map）
    //   Boss 实际 spawn 已迁移到 boss_appeared 路径（L3253）— 由 _enterNight 直接写 _activeMonsters
    //   normal/elite 由 _spawnWave 按波次写 _activeMonsters（_earlyDayMult 已应用在两路径中）
    //   _difficulty 字段已废止（v1.27），ledger 日志不再读 difficulty
    ...
    console.log(`[SurvivalEngine] Night ${day} monster ledger reset: normal~${expectedNormal} elite~${expectedElite} boss=${cfg.boss ? 1 : 0} (fd=${this.fortressDay} earlyMult=${earlyMult.toFixed(3)} hpMult=${hpMult.toFixed(3)} cntMult=${cntMult.toFixed(3)})`);
  }
  ...
  _spawnWave(cfg, day, waveIndex) {
    ...
    // §14 v1.27 + codex review：废止 _monsterHpMult 后乘数链 = _dynamicHpMult × _themeHpMult × themeHpMult(modifier polar_night 层) × _actEndHpBaseMult × _earlyDayMult
    //   r16 补 _themeHpMult（赛季主题层 blood_moon=1.2）保留；codex review §34.4 E2 _actEndHpBaseMult 幕末乘数保留
    //   _earlyDayMult 由 worktree v1.27 引入（earlyMult 局部变量已在 _spawnWave 顶部声明）
    const baseHp       = Math.max(1, Math.round(cfg.normal ? cfg.normal.hp * (this._dynamicHpMult || 1.0) * (this._themeHpMult || 1.0) * themeHpMult * (this._actEndHpBaseMult || 1.0) * earlyMult : 50));
  ...
  onSeasonStarted(newSeasonId) {
    ...
    // §14 v1.27：废止 _pendingDifficulty 字段后，原 codex review 的"延迟到下赛季应用 difficulty"路径已不存在
    //   onSeasonStart 仍保留 noop 调用（外部插件 / 测试兼容）
  ...
  _spawnExtraBossForActEnd(day) {
    ...
    if (!cfg.boss) return;
    const themeHpMult = this._themeHpMult || 1.0;
    // §14 v1.27 + codex review：废止 _monsterHpMult 后链 = _dynamicHpMult × _themeHpMult × _themeBossHpMult × _actEndHpBossMult × _earlyDayMult
    //   r17 _themeBossHpMult 保留（serene 主题 act1 双 Boss 一致性）；codex §34.4 E2 _actEndHpBossMult 保留
  ...
  _applyNightModifier(mod, day) {
    ...
    switch (mod.id) {
      case 'blood_moon': {
        ...
        const cfg = getWaveConfig(day);
        this._activeMonsters.clear();
        // codex review：补 _lastBossSpawnSide（blood_moon 也共享 _withMonsterSpawnMeta side 逻辑）
        const sides = ['left', 'right', 'top', 'all'];
        this._lastBossSpawnSide = sides[Math.floor(Math.random() * sides.length)];
        // §14 v1.27：blood_moon Boss / elite HP 链中也叠 _earlyDayMult（早期日渐进保护）
        ...
        if (cfg.elite) {
          // §14 v1.27：废止 _monsterHpMult 后链 = _dynamicHpMult × _themeHpMult × _earlyDayMult（elite 不叠 _themeBossHpMult，那只用于 Boss 派生层）
          //   r17 GAP-R17-PM-03 _themeHpMult 保留（与 _spawnWave / _initActiveMonsters elite 同形态对齐）
